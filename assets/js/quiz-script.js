...
function fnDisplayHighScores() {
    ...
    if (strStoresUserDetls != null) { //only continue if there was data to use and display stuff on highscore board
        for (let index = 0; index < strStoresUserDetls.length; index++) { //loop over every array element found (highscore entry)
            ...
            liUserHighScore.textContent = strStoresUserDetls[index].names + ` - ` + strStoresUserDetls[index].scores; //fill up new <li> with content of stored highscores
...
function fnStartTimer() {
    var timerInterval = setInterval(function () {
        ...
        lblTimer.textContent = intTimeLeft; //update timer tag to latest time
...
function fnQuizIntialize() {
    ...
    intTimeLeft = intGlobalIntialTimer; //reset the time back to 99 seconds so reusable to reset game
    lblTimer.textContent = intGlobalIntialTimer; //go in and set the default number of the timer so it starts at the actual start number on page load
    // Hide unwanted items
